...
const AllOrder = () => {
  ...
  const handleUpdate = (id) => {
    setUpdateLoading(id);
    axios
      .put(`https://tour-planner-backend.herokuapp.com/confirmOrder/${id}`)
      .then((res) => {
        if (res.status === 200) {
          setControlFetch(!controlFetch);
        }
  ...
  const backgroundStyle = {
    background: `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.4)), url("https://images.unsplash.com/photo-1470770903676-69b98201ea1c?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1170&q=80")`,
  ...
  return (
    <>
      ...
      <Container sx={{ my: 5 }}>
        <HeadingText>All Orders</HeadingText>
        {loading ? (
          <Preloader></Preloader>
        ) : orders.length !== 0 ? (
          <>
            <Grid container>
              ...
              >
                <GridItemLeft>Ordered By</GridItemLeft>
              </Grid>
              <Grid
                xs={3}
                style={{
              ...
              >
                <GridItemLeft>Manage Order</GridItemLeft>
              </Grid>
            </Grid>
            ...
            {orders.map((order, i) => (
              <Grid container key={order._id}>
                ...
                >
                  <GridItemRight>{order.name}</GridItemRight>
                </Grid>
                <Grid
                  xs={3}
                  style={{
